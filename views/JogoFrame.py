...
class JogoFrame:
    ...
    def __init__(self, frame: MyFrame, torto: Torto) -> None:
        ...
        self._palavra_frame = PalavraComponent(self._frame, self._checar_palavra)
        self._palavra_frame.frame.grid(row=1, column=0)

        self._my_frame.manter_aberto()
    
    def _checar_palavra(self, event = None) -> None:
        try:
            palavra = self._palavra_frame.palavra_entry.get()
            self._torto.submeter_palavra(palavra, self._celulas_escolhidas)
            TortoController().checar(self._my_frame, self._torto)
